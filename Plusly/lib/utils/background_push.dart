...
class BackgroundPush {
  ...
  void _init() async {
    ...
    try {
      ...
      if (Platform.isAndroid) {
        Logs().i('[Push] Initializing UnifiedPush...');
        ...
        Logs().i('[Push] UnifiedPush initialized successfully');
      }
    } catch (e, s) {
  ...
  Future<void> setupPusher({
    ...
    if (setNewPusher) {
      try {
        ...
        Logs().i('[Push] ✅ Pusher registered successfully!');
        Logs().i('[Push] Endpoint: $gatewayUrl');
        Logs().i('[Push] App ID: $thisAppId');
      } catch (e, s) {
        Logs().e('[Push] ❌ Unable to set pushers', e, s);
      }
    } else {
      Logs().i('[Push] Pusher already set (no update needed)');
    }
  }
  ...
  Future<void> _onUpMessage(PushMessage pushMessage, String i) async {
    Logs().wtf('[Push] 📬 Push Notification received from UnifiedPush!', pushMessage);
